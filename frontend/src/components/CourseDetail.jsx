...
export default function CourseDetail({ course, onSessionToggle, onSessionDelete, onAddExtraSession }) {
  ...
  const handleAddSubmit = (week, type) => {
    if (onAddExtraSession) onAddExtraSession(course.id, week, type);
  ...
  const allWeeks = useMemo(() => {
    ...
    (course.sessions || []).forEach((s) => { if (s.week > maxWeek) maxWeek = s.week; });
  ...
  const filteredWeeks = useMemo(() => {
    const weeksToShow = typeof weekFilter === 'number' ? [weekFilter] : allWeeks;
    return weeksToShow
      .map((week) => {
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="bg-white dark:bg-slate-800 border border-gray-200 dark:border-slate-700 rounded-2xl p-6 shadow-sm dark:shadow-lg">
        <div className="flex items-start justify-between mb-4">
          <div>
            <div className="text-sm text-gray-500 dark:text-slate-400 mb-1">התקדמות בקורס</div>
            <div className="text-3xl font-bold text-gray-900 dark:text-white">{pct}%</div>
          </div>
          <div className="text-left">
            <div className="text-sm text-emerald-600 dark:text-emerald-400 font-semibold">{watched} הושלמו</div>
            <div className="text-sm text-gray-500 dark:text-slate-400">{total - watched} נותרו</div>
          </div>
        </div>
        <div className="progress-bar h-3">
          <div
            ...
            style={{
              width: `${pct}%`,
              backgroundColor: pct === 100 ? '#10b981' : color.hex,
            }}
          />
        </div>
        <div className="mt-4 grid grid-cols-3 gap-3 text-center text-sm">
          <div className="bg-gray-100/80 dark:bg-slate-700/50 rounded-xl py-2">
            <div className="font-bold text-gray-900 dark:text-white">{course.totalWeeks}</div>
            <div className="text-xs text-gray-500 dark:text-slate-400">שבועות</div>
          </div>
          <div className="bg-gray-100/80 dark:bg-slate-700/50 rounded-xl py-2">
            <div className="font-bold text-indigo-600 dark:text-indigo-300">{course.weeklyLectures}</div>
            <div className="text-xs text-gray-500 dark:text-slate-400">הרצאות/שבוע</div>
          </div>
          <div className="bg-gray-100/80 dark:bg-slate-700/50 rounded-xl py-2">
            <div className="font-bold text-violet-600 dark:text-violet-300">{course.weeklyTutorials}</div>
            <div className="text-xs text-gray-500 dark:text-slate-400">תרגולים/שבוע</div>
          </div>
        </div>
      </div>
        ...
        {weekFilter !== 'remaining' && (
          <div className="flex flex-wrap gap-2 items-center">
            ...
            <button
              ...
              className="px-4 py-1.5 rounded-full text-sm font-medium border border-gray-300 dark:border-slate-600 text-gray-600 dark:text-slate-300 hover:bg-gray-100 dark:hover:bg-slate-700 hover:text-gray-900 dark:hover:text-white transition-all mr-auto"
            >
              + הוסף חריג
            </button>
          </div>
...
function AddExtraSessionModal({ course, defaultWeek, onClose, onSubmit }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
      <div className="bg-white dark:bg-slate-800 border border-gray-200 dark:border-slate-700 rounded-2xl w-full max-w-sm shadow-2xl">
        <div className="flex items-center justify-between p-5 border-b border-gray-200 dark:border-slate-700">
          <h2 className="text-lg font-bold text-gray-900 dark:text-white">הוספת שיעור חריג</h2>
          <button onClick={onClose} className="text-gray-400 dark:text-slate-400 hover:text-gray-900 dark:hover:text-white transition-colors p-1 rounded">
        ...
        <form onSubmit={handleSubmit} className="p-5 space-y-4">
          <div>
            <label className="block text-sm font-medium text-gray-600 dark:text-slate-300 mb-1.5">הוסף לשבוע</label>
            <input
              type="number"
              min={1}
              max={50}
              ...
              className="w-full bg-gray-100 dark:bg-slate-700 border border-gray-300 dark:border-slate-600 rounded-xl px-4 py-2.5 text-gray-900 dark:text-white outline-none focus:border-indigo-500"
            />
          </div>
          <div>
            <label className="block text-sm font-medium text-gray-600 dark:text-slate-300 mb-1.5">סוג שיעור</label>
              ...
              <button
                ...
                className={`flex-1 py-2 rounded-xl text-sm font-medium border transition-colors ${
                  type === 'lecture'
                    ...
                    : 'bg-gray-100 dark:bg-slate-700 border-gray-300 dark:border-slate-600 text-gray-600 dark:text-slate-300 hover:bg-gray-200 dark:hover:bg-slate-600'
                }`}
              >
                הרצאה
              </button>
    ...
    >
      {pills.map(({ key, label }) => {
        ...
        return (
          ...
          >
            {isRemaining && <span className="ml-1">⚑</span>}
  ...
  if (weekFilter === 'remaining') {
    return (
      <div className="text-center py-12 text-gray-400 dark:text-slate-400">
        ...
        <div className="font-medium text-gray-600 dark:text-slate-300">כל המשימות הושלמו!</div>
  ...
  return (
    <div
      className={`bg-white dark:bg-slate-800 border rounded-2xl overflow-hidden shadow-sm transition-colors ${
        weekDone ? 'border-emerald-500/30' : 'border-gray-200 dark:border-slate-700'
      }`}
    >
      <button
      ...
      >
        <div className="flex items-center gap-3">
          ...
          <span className="font-semibold text-gray-900 dark:text-white">שבוע {week}</span>
          {weekDone && (
            <span className="text-xs text-emerald-600 dark:text-emerald-400 bg-emerald-500/10 border border-emerald-500/20 px-2 py-0.5 rounded-full">
              הושלם
            </span>
          )}
        </div>
        <div className="flex items-center gap-3">
          <span className="text-xs text-gray-400 dark:text-slate-400">{weekWatched}/{weekTotal}</span>
          <svg
      ...
      {open && (
        sessions.length === 0 ? (
          <div className="px-5 py-4 flex flex-col items-center justify-center gap-2 border-t border-gray-200 dark:border-slate-700">
            <div className="text-sm text-gray-400 dark:text-slate-500">אין שיעורים בשבוע זה</div>
            {onAddExtra && (
              <button
                onClick={onAddExtra}
                className="text-xs bg-gray-100 dark:bg-slate-700 hover:bg-gray-200 dark:hover:bg-slate-600 text-gray-600 dark:text-slate-300 px-3 py-1.5 rounded-lg transition-colors border border-gray-300 dark:border-slate-600"
              >
                + הוסף שיעור
              </button>
            )}
          ...
          <div className="border-t border-gray-200 dark:border-slate-700 divide-y divide-gray-100 dark:divide-slate-700/50">
            ...
            {onAddExtra && (
              <div className="px-5 py-3 bg-gray-50/50 dark:bg-slate-800/30">
                <button
                  onClick={onAddExtra}
                  className="text-xs text-gray-400 dark:text-slate-400 hover:text-indigo-600 dark:hover:text-indigo-400 transition-colors flex items-center gap-1 font-medium"
                >
...
function SessionRow({ session, onToggle, onDelete }) {
  ...
  return (
    <label className="flex items-center gap-4 px-5 py-3.5 cursor-pointer hover:bg-gray-50 dark:hover:bg-slate-700/30 transition-colors group">
      ...
      <div className="flex-1 flex items-center gap-3">
        ...
        <span className={`text-sm ${session.watched ? 'line-through text-gray-400 dark:text-slate-500' : 'text-gray-700 dark:text-slate-200'}`}>
